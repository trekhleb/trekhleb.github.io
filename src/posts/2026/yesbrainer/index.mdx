...
summary: "Yes-Brainer is a council of AI models for the decisions that aren't no-brainers. One question fans out to several models — they answer in parallel, debate to consensus, or get judged to a verdict. No backend, no accounts: your keys, your browser."
...
![Yes-Brainer](./assets/01-cover.jpg)

For non-trivial questions — the ones that are either complex or important — I caught myself in a "ritual": copy-pasting the same prompt into Claude, then Gemini, then ChatGPT, in three browser tabs, and eyeballing the differences.
...
- 🔗 Try it: [yesbrainer.ai](https://yesbrainer.ai)
- 🔗 Source code: [github.com/trekhleb/yesbrainer](https://github.com/trekhleb/yesbrainer)

One question fans out to several models at once, and instead of juggling tabs you get a deliberation in one place:

- 🔀 **Parallel** — independent answers, side by side
- ⚖️ **Trial** — the models vote anonymously on each other's answers, then a judge synthesizes a verdict
- 🤝 **Consensus** — a real multi-round debate, with a mediator that either drives it to convergence or honestly reports what stayed contested
...
![Yes-Brainer front page](./assets/02-home.jpg)
...
![Creating a new council](./assets/03-new-council.jpg)
...
![The council convenes](./assets/04-council-convenes.jpg)
...
![Parallel: three models naming the app, side by side](./assets/05-parallel-naming.png)

### ⚖️ Trial — anonymous peer votes, then a verdict
...
![Trial: where was this photo taken](./assets/06-trial-photo.jpg)
...
![Trial: votes and the verdict](./assets/07-trial-verdict.jpg)
...
### 🤝 Consensus — a genuine multi-round debate
...
![Consensus reached, with shifted and held chips](./assets/08-consensus-reached.jpg)
...
![Consensus share card](./assets/09-consensus-share-card.png)
...
- **Your API keys live in your browser** (localStorage) and travel exactly one route: directly from your browser to the provider you chose. You pay only your model providers. There's no server of mine in between — so there's nothing for me to leak.
- **Your conversations live on your device** (IndexedDB). Nobody holds a copy; JSON export/import is the manual bridge between devices.
- The production build ships a strict **Content-Security-Policy** whose allowlist names exactly the supported provider endpoints — even if malicious code somehow ran in the page, the browser would refuse to send your keys anywhere else.
- Provider errors are **scrubbed of secrets** before they're stored or logged — an SDK error that echoes your auth header persists as `[redacted]`.
- The app is **open source under AGPL-3.0**. "Read the code you're pasting keys into" is the trust story, and the AGPL network clause keeps every hosted fork readable too.

![Settings — Keys](./assets/10-keys.jpg)

Your part of the deal (the app says this right where you paste the key): check that the address bar says **yesbrainer.ai** — a copied site can fake everything except its URL — and use a dedicated key with a spending limit set in the provider's console. Two honest footnotes: prompts sent to OpenRouter models pass through OpenRouter's gateway (that's what its one-key-for-many-vendors convenience buys), and the official site sends a single cookieless pageview ping to a self-hosted counter with a closed payload — no third-party scripts, forks send nothing, and there's an opt-out. The full threat model, including what the code *can't* protect against, is in [SECURITY.md](https://github.com/trekhleb/yesbrainer/blob/main/SECURITY.md).
...
![Yes-Brainer on a phone](./assets/11-mobile.png)

## Standing on the shoulders of llm-council

The idea is not new, and the starting point deserves proper credit: [llm-council](https://github.com/karpathy/llm-council) by [Andrej Karpathy](https://twitter.com/karpathy) is a great experiment that runs an "answer → review → synthesize" pass over several models, and its orchestration concept and prompt designs (judge synthesis, anonymized peer evaluation) are where this project started.

What I pushed further is the shape:

- **Three deliberation structures instead of one fixed pipeline** — including the multi-round Consensus debate refereed to convergence, which is the part I haven't seen elsewhere.
- **Follow-ups** — a council is a conversation, not a one-shot; every follow-up carries the full context to every seat.
- **No server at all** — llm-council (and most of its 80+ forks) needs a local server you run; Yes-Brainer is a static bundle that works on your phone.
- **The disagreement made visible** — the vote spread, the agreement indicators, the "who shifted / who held" chips. The app shows you where the models split instead of smoothing it over.

The multi-LLM space is busy in general: there are hosted council products (subscription-based, with your conversations on their servers) and multi-model aggregators (side-by-side answers without a deliberation mechanic). The combination here — open source, zero backend, bring-your-own-keys, an iterative debate, on-device data — is a slot I haven't found occupied. If you know a closer neighbor, [open an issue](https://github.com/trekhleb/yesbrainer/issues) — I keep the comparison section of the README honest.

## More models isn't truth
...
👉 **[yesbrainer.ai](https://yesbrainer.ai)** — the recorded demo councils are one click away, no keys needed. Paste a key (or enable local Ollama) and those same councils become interactive — you can keep asking them follow-ups.

It's a side project, so there will be rough edges. If you hit one, [issues and feedback](https://github.com/trekhleb/yesbrainer/issues) are very welcome. And if you want to poke at how it's built — the whole thing is open: [github.com/trekhleb/yesbrainer](https://github.com/trekhleb/yesbrainer).
