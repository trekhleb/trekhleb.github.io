...
The cars are hitting some other cars along the way, bit this is only the 40th generation since the creation of the world for them, so you may give the cars some more time to learn.

From generation to generation we may see how the loss values are going down. The `P50 Avg Loss` shows the average loss value (average distance from the cars to the parking lot) of the `50%` most fittest cars. The `Min Loss` shows the loss value of one most fittest car in each generation.

![Loss history](assets/10-loss-history-02.png)

You may see that in average the `50%` of the most fittest cars of the generation are learning to get closer to the parking lot (from `5.5m` away from the parking lot to `3.5m` in 35 generations). The trend for the `Min Loss` values is less obvious (from `1m` to `0.5m` with some noise signals), but from the animations above you may clearly see that cars have learned basic parking moves.
