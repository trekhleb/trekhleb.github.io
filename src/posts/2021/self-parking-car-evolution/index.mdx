...
To get the fitness values of each car we would need a simulation of the cars behavior running in a virtual 3D world. You may check the [Evolution Simulator](https://trekhleb.dev/self-parking-car-evolution) to see the code below in action:
...
After running the `select()` function, the generation array is sorted by the fitness values in a descending order. Therefore, after 40 generations the fittest car will be the first car in the generation array (`generation[0]`).
