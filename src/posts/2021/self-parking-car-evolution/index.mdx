...
Step-by-step we're going to brake down a high-level task of creating the self-parking car to the straightforward low-level optimization problem of finding the optimal combination of `180` bits (finding the optimal car genome).
...
3. 🧠 Give the **brain** to the car that will control the muscles (movements) based on what car sees (obstacles via sensors). The brain will be simply a pure function `movements = f(sensors)`.
...
However, the purpose of this article was to have some fun learning how the genetic algorithm works and not to build a production-ready self-parking Tesla of course. So, even with the issues mentioned above, I hope you've had a good time going through the article.
