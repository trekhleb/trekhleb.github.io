...
```typescript
const sensors = [s0, s1, s2, s3, s4, s5, s6, s7];
...
Each number might take one of three values: `-1`, `0`, or `+1`. For example:

```typescript
// i.e. { engineSignal: 0, wheelSignal: -1 } ← 🧠
```
...
However, to avoid the introduction of a whole new concept of Neural Networks, we'll go with a much simpler approach and we'll use two simple formulas for the car's brain:
...
Where `engineBrainFunction` and `wheelBrainFunction` are functions that implement simple linear equations:
...
wheelSignal = ((w0 * s0) + (w1 * w1) + ... + (w7 * w7)) + w8

// Where:
// [s0, s1, ..., s7] - are 8 sensor values
// [e0, e1, e2, ..., e8] - are 9 engine formula coefficients
// [w0, w1, w2, ..., w8] - are 9 wheel formula coefficients
```
