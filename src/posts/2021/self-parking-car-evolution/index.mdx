...
Lets put the evolution functions together. We're going to launch the evolution loop, make the time going, the generation evolving, and the cars learning how to park.

To get the fitness values of each car we would need to run a simulation of the cars behavior in a virtual 3D world. The [Evolution Simulator](https://trekhleb.dev/self-parking-car-evolution) does exactly that - it runs the code below in the simulator:
...
After running the `select()` function, the generation array is sorted by the fitness values in a descending order. Therefore, the fittest car will be the first car in the `generation` array.
...
![Loss history](assets/10-loss-history-02.png)
