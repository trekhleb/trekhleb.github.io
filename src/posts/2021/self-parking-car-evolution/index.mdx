...
To get the fitness values of each car we need to run a simulation of the cars behavior in a virtual 3D world. The [Evolution Simulator](https://trekhleb.dev/self-parking-car-evolution) does exactly that - it runs the code below in the simulator:
...
const carFitnessFunction = (genome: Genome): number => {
  // The evolution simulator calculates and stores the fitness values
  // for each car in the fitnessValues map.
...
After running the `select()` function, the `generation` array is sorted by the fitness values in a descending order. Therefore, the fittest car will be the first car in the array.

**The 1st generation** of cars with random genomes will behave something like this:

![The 1st generation of cars with random genomes](assets/02-cars-before-01.gif)

**On the ≈40th generation** the cars start learning what the self-parking is and start getting closer to the parking lot:
...
From generation to generation we may see how the loss values are going down. The `P50 Avg Loss` shows the average loss value (average distance from the car to the parking lot) of the `50%` of the most fittest cars. The `Min Loss` shows the loss value of the most fittest car in each generation.

![Loss history](assets/10-loss-history-02.png)

You may see that on the `35th` generation the `50%` of most fittest cars are learning to get closer to the parking lot - from `5.5m` away from the parking lot (on the 0th generation), to `3.5m` (on the 35th generation). The trend for the `Min Loss` values is less obvious (from `1m` to `0.5m` with some noise signals), but from the animations above you may clearly see that cars have learned some parking behavior.
