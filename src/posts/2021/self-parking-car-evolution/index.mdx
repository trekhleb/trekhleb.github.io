...
Where `brainToMuscleSignal()` is just a function that converts raw brain signals (any float number) to muscle signals (to  `-1`, `0`, or `+1` number) so that muscles could understand it. We'll implement this converter function below.
...
Let's get back to the `brainToMuscleSignal()` and implement it. The output of a `linearPolynomial()` function is a float number (i.e. `0.456` or `3673.45` or `-280`). The `brainToMuscleSignal()` function need to convert the wide range of floats to three particular integers, and it will do it in two steps:

1. Convert the float of a wide range (i.e. `0.456` or `3673.45` or `-280`) to the float in a range of `(0...1)` (i.e. `0.05` or `0.86`)
2. Convert the float in a range of `(0...1)` to one of three integer values of `-1`, `0`, or `+1`.  Floats that are close to `0` will be converted to `-1`, floats that are close to `0.5` will be converted to `0`, and floats that are close to `1` will be converted to `1`.

To do the first part of the conversion we need to introduce a [Sigmoid](https://en.wikipedia.org/wiki/Sigmoid_function):

![Sigmoid formula](assets/05-sigmoid-01.svg)

It converts the wide range of floats (the `x` axis) to a limited range of `(0...1)` (the `y` axis). This is exactly what we need.

![Sigmoid graph](assets/05-sigmoid-02.png)

The implementation of two conversion steps mentioned above would look like this then:
...
const sigmoidToMuscleSignal = (
  sigmoidValue: number,
  // Value between 0 and 0.5:  [0 ... (0.5 - margin) ... 0.5 ... (0.5 + margin) ... 1]
  aroundZeroMargin: number = 0.4,
): MuscleSignal => {
