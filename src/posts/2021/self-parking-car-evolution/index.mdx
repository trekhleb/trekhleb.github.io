...
You may see that in average the `50%` of most fittest cars of the generation are learning to get closer to the parking lot (from `5.5m` away from the parking lot to `3.5m` in 35 generations). The trend for the `Min Loss` values is less obvious (from `1m` to `0.5m` with some noise signals), but from the animations above you may clearly see that cars have learned some parking behavior.
