...
Now, let's create the world, make the time going, generation evolving, and the cars learning how to park:

```typescript
// Configurable evolution setup.
...
const carFitnessFunction = (genome: Genome): number => {
  // To make the fitness function work we need a simulation of the car behavior in a virtual 3D world.
...
while(generationIndex < MAX_GENERATIONS_NUM) {
  // SIMULATION IS NEEDED HERE to simulate each car in the virtual world
  // and to calculate the fitness values.
  // @see: https://trekhleb.dev/self-parking-car-evolution

  // Create the next generation based on the fitness values of the current generations.
  // Mating and mutation will happen inside the select() function.
  generation = select(
    generation,
    carFitnessFunction,
    {
...
// After selection the generation array is sorted by the fitness values in a descending order.
// Therefore, after 40 generations the fittest car will be the first car in the generation array.
```

> As you've notice, we need to simulate the cars in the virtual 3D world, to calculate the fitness values. You might want to launch the [Evolution Simulator](https://trekhleb.dev/self-parking-car-evolution) to see the code above in action.
...
What is important is that from generation to generation we may see how the loss values are getting down.
