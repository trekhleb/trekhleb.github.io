...
The image that needs to be resized is being passed to the function in [ImageData](https://developer.mozilla.org/en-US/docs/Web/API/ImageData) format. You may draw the image on the canvas and then extract the ImageData from canvas like this:
...
Here we apply the color difference formula described above. For the left and right borders (when there is no left or right neighbors) we ignore the neighbors and don't take them into account during the energy calculation.
...
The image we're working with is in the [ImageData](https://developer.mozilla.org/en-US/docs/Web/API/ImageData) format. It means that all the pixels (and their colors) are stored in flat (*1D*) [Uint8ClampedArray](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Uint8ClampedArray) array. For readability purposes let's introduce the couple of helper functions that will allow us to work with the Uint8ClampedArray array as with a *2D* matrix instead.
...
> I've already went through some Dynamic Programming basics in [Dynamic Programming vs Divide-and-Conquer](https://trekhleb.dev/blog/2018/dynamic-programming-vs-divide-and-conquer/) article before. There is a DP example based on the minimum edit distance problem. You might want to check it out to give some more context.
...
Going from top to bottom, for each pixel we have 3 options (↙︎ go down-left, ↓ go down, ↘︎ go down-right). This gives us the time complexity of `O(3^n)` which looks slow.

#### The greedy approach

We may also try to choose the next pixel as a pixel with the lowest energy, hoping that resulting seam energy will be the smallest one.

![The greedy approach](assets/51-greedy-approach.png)

This approach gives not the worst solution, but it cannot guarantee that we will find the best available solution. On the image above you may see how greedy approach chose `5` instead of `10` at first and missed the chain of optimal pixels.

The good part about this approach is that it is fast, and it has time complexity of `O(n)`. In this case the cost of the speed is low quality of resizing.
...
So, we may **save the energy of the current seam** at particular pixel in additional `seamsEnergies` table to make it re-usable for calculating the next seams faster (the `seamsEnergies` table will have the same size as the energy map and the image itself).
...
Since we're looking for a seam with the lowest resulting energy possible it makes sense to pick the previous seam with the lowest resulting energy as well.
...
Here is how the algorithm works for objects removal.
