...
![Energy map sketch](assets/40-energy-map.png)

We may use the energy map to find a seam with the lowest energy. To do so, we need to find a path (the continuous sequence of pixels) so that the sum of energies of all pixels would be the lowest one.

![Energy map example](assets/40-energy-map-with-seam.png)
