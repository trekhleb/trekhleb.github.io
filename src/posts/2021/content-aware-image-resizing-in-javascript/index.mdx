...
Now, since we know how to find the energy of one pixel, we can calculate, so called, **energy map** which will contain the energies of each pixel of the image. On each resizing step the energy map has the same size as the image. For example, on the first resizing step we will have a `1000 x 500 px` image and `1000 x 500` energy map.
