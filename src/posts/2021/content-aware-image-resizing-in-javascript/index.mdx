...
> The way of uploading and drawing images in JavaScript is out of scope for this article, but you may find the complete source-code of how it may be done using React in [js-image-carver repo](https://github.com/trekhleb/js-image-carver).
...
Here we just apply the color difference formula described above. For the left and right borders (when there is no left or right neighbors) we simply ignore the neighbors and don't take them account during the energy calculation.
