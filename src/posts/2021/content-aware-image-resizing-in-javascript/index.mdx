...
Imagine we have a picture of size `1000 x 600 px` and we want to change its size to `600 x 600 px` to make it a square (let's say it better fits the Instagram feed). While shrinking the image, we also don't want to lose some important parts of the content (i.e. we want to preserve the fact that the car's wheels have a circle shape and not an ellipse one).

To do so, we may find the *continuous sequence of pixels (the seam)*, that goes from top to bottom and has the lowest contribution to the content of the image and then remove it. The seam removal will shrink the image by 1 pixel. We will then repeat these steps until the image will become square.

The question is how to define *the importance of the pixel* and its contribution to the content (in the original paper the authors are using the term *energy* for the importance). One of the ways to do it is to treat all the pixels that form the edges as important ones. In case if a pixel is a part of the edge its color would have a greater difference between the neighbors (left and right pixels) than the pixel that isn't a part of the edge. Assuming that the color of a pixel is represented by *4* numbers (`R` - red, `G` - green, `B` - blue, `A` - alpha) we may use the following formula to calculate the difference:

![Pixel energy formula](assets/20-energy-formula.png)

Where:

- `mEnergy` - *Energy* (importance) of the *middle* pixel
- `lR` - *Red* channel value for the *left* pixel
- `mR` - *Red* channel value for the *middle* pixel
- `rR` - *Red* channel value for the *right* pixel
- `lG` - *Green* channel value for the *left* pixel
- and so on...
